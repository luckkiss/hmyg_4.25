package com.hldj.hmyg.MyLuban;/*Copyright 2016 Zheng Zibin
...
public class MyLuban {
    ...
    private MyLubanBuilder mBuilder;

    private MyLuban(File cacheDir) {
        mBuilder = new MyLubanBuilder(cacheDir);
    }

    public static MyLuban compress(Context context, File file) {
        MyLuban luban = new MyLuban(MyLuban.getPhotoCacheDir(context));
        luban.mFile = file;
        luban.mFileList = Collections.singletonList(file);
        return luban;
    }

    public static MyLuban compress(Context context, List<File> files) {
        MyLuban luban = new MyLuban(MyLuban.getPhotoCacheDir(context));
     ...
     */
    public static MyLuban compress(File file, File cacheDir) {
        if (!isCacheDirValid(cacheDir)) {
            throw new IllegalArgumentException("The cacheDir must be Directory");
        }
        MyLuban luban = new MyLuban(cacheDir);
        luban.mFile = file;
        luban.mFileList = Collections.singletonList(file);
        return luban;
    }

    public static MyLuban compress(List<File> files, File cacheDir) {
        if (!isCacheDirValid(cacheDir)) {
            throw new IllegalArgumentException("The cacheDir must be Directory");
        }
        MyLuban luban = new MyLuban(cacheDir);
     ...
     */
    public MyLuban putGear(@GEAR int gear) {
        mBuilder.gear = gear;
        return this;
    }
     ...
     * 自定义图片压缩格式
     */
    public MyLuban setCompressFormat(Bitmap.CompressFormat compressFormat) {
     ...
     */
    public MyLuban setMaxSize(int size) {
     ...
     * @param width 最大宽度
     */
    public MyLuban setMaxWidth(int width) {
     ...
     * @param height 最大高度
     */
    public MyLuban setMaxHeight(int height) {
    ...
    public Observable<File> asObservable() {
        MyLubanCompresser compresser = new MyLubanCompresser(mBuilder);
    ...
    private static File getPhotoCacheDir(Context context) {
        return getPhotoCacheDir(context, MyLuban.DEFAULT_DISK_CACHE_DIR);
    }
