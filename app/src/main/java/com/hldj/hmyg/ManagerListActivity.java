...
public class ManagerListActivity extends NeedSwipeBackActivity implements IXListViewListener,
...
//        findTagTextView(linearLayout);
    ...
    List<View> list_counts = new ArrayList<>();
        ...
        for (int i = 0; i < viewGroup.getChildCount(); i++) {
            if (viewGroup.getChildAt(i) instanceof ViewGroup) {
                ...
                if (viewGroup.getChildAt(i).getTag() != null && viewGroup.getChildAt(i).getTag().toString().equals("tag")) {
                    list_counts.add(viewGroup.getChildAt(i));
                }
            }
        }
                                    ...
                                    if (listAdapter == null) {
                                        ...
                                        xListView
                                                .setOnItemClickListener((arg0, arg1, position, arg3) -> {
                                                    ...
                                                    // method stub
                                                    if (position != 0) {
                                                        Intent toFlowerDetailActivity = new Intent(ManagerListActivity.this, FlowerDetailActivity.class);
                                                        toFlowerDetailActivity
                                                                .putExtra("id", datas.get(position - 1).get("id").toString());
                                                        toFlowerDetailActivity
                                                                .putExtra("show_type", datas.get(position - 1).get("show_type").toString());
...
//                        break;
    ...
    public void onRefresh() {
        ...
        datas.clear();
        if (listAdapter == null) {
            listAdapter = new ProductListAdapterForManager(ManagerListActivity.this, datas);
            xListView.setAdapter(listAdapter);
        } else {
            listAdapter.notifyDataSetChanged();
        }
