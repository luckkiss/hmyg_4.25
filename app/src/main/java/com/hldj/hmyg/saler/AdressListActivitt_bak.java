...
public class AdressListActivitt_bak extends NeedSwipeBackActivity implements IXListViewListener {
        ...
        if (imm.isActive()) {
            if (getCurrentFocus() != null) {
                if (getCurrentFocus().getWindowToken() != null) {
                    imm.hideSoftInputFromWindow(mActivity
            ...
            if (flag) {
                switch (view.getId()) {
                    ...
                    case R.id.iv_add_adress:
                        Intent toAddAdressActivity = new Intent( mActivity, AddAdressActivity.class);
...
//                        break;
                ...
                inflate.setOnClickListener(v -> {
                    if (SaveSeedlingActivity.instance != null) {
                        if ("".equals(data.get(position).get("twCode").toString())) {
                            Toast.makeText(mActivity, "请编辑完整这条地址的所在街道",
                                    Toast.LENGTH_SHORT).show();
                            return;
                        }
                    }
                    Address address = new Address();
                    address.addressId = data.get(position).get("id").toString();
                    address.contactPhone = data.get(position).get("contactPhone").toString();
                    address.contactName = data.get(position).get("contactName").toString();
                    address.cityName = data.get(position).get("cityName").toString();
                    address.isDefault = (boolean) data.get(position).get("isDefault");
            ...
            iv_edit.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View arg0) {
                    ...
                    Intent toAddAdressActivity = new Intent(
                            mActivity, AddAdressActivity.class);
                    ...
                    toAddAdressActivity.putExtra("type", data.get(position) .get("type").toString());
                    toAddAdressActivity.putExtra("mainType", data.get(position).get("mainType").toString());
                    startActivityForResult(toAddAdressActivity, 1);
    ...
    public void onRefresh() {
        ...
        datas.clear();
        if (ftadapter == null) {
            ftadapter = new MyAdapter(mActivity, datas);
            xListView.setAdapter(ftadapter);
        } else {
            ftadapter.notifyDataSetChanged();
        }
    ...
    public static void start2AdressListActivity(Context context, String addressId, String from, SaveSeedingBottomLinearLayout.onAddressSelectListener onAddressSelectListener) {
        Intent intent = new Intent(context, AdressListActivitt_bak.class);
        intent.putExtra("addressId", addressId);
        intent.putExtra("from", from);
        AdressListActivitt_bak.onAddressSelectListener = onAddressSelectListener;
        context.startActivity(intent);
    }


    public static void start2Activity(Context mActivity) {
        Intent intent = new Intent(mActivity, AdressListActivitt_bak.class);
//        intent.putExtra("from", from);
