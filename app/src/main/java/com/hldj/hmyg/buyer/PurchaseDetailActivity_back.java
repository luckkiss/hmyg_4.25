...
@SuppressLint("NewApi")
public class PurchaseDetailActivity_back extends BaseThirdActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		hud = KProgressHUD.create(PurchaseDetailActivity_back.this)
	...
	private void initSearch() {
		...
		if (planttype_names.size() > 0) {
			...
			adapter3 = new TagAdapter<String>(
					planttype_names) {
		...
		finalHttp.post(GetServerUrl.getUrl() + url, params,
				new AjaxCallBack<Object>() {
						...
						try {
							...
							if ("1".equals(code)) {
								...
								if ("".equals(num) && "".equals(purchaseFormId)) {
									...
									id_tv_edit_all.setVisibility(View.GONE);
									id_tv_edit_all
											.setOnClickListener(PurchaseDetailActivity_back.this);
								}
								...
								info_02.setOnClickListener(PurchaseDetailActivity_back.this);
								...
								if (specList.length() > 0) {
									...
									if (msSeedlingParms.size() > 0) {
										myadapter = new PurchaseSeedlingParmAdapter(
												PurchaseDetailActivity_back.this,
												msSeedlingParms);
								...
								info_01.setOnClickListener(PurchaseDetailActivity_back.this);
					...
					public void onFailure(Throwable t, int errorNo,
							String strMsg) {
						// TODO Auto-generated method stub
						Toast.makeText(PurchaseDetailActivity_back.this,
	...
	public void onClick(View v) {
		...
		case R.id.info_01://采购商信息
			...
			break;
		case R.id.ll_login:
			Intent toLoginActivity = new Intent(PurchaseDetailActivity_back.this,
					LoginActivity.class);
			startActivityForResult(toLoginActivity, 4);
			overridePendingTransition(R.anim.slide_in_left,
					R.anim.slide_out_right);
		...
		case R.id.ll_01:
			...
			new ActionSheetDialog(PurchaseDetailActivity_back.this)
					.builder()
					.setCancelable(false)
		...
		case R.id.save:
			...
			if (imm.isActive() && getCurrentFocus() != null
				...
				imm.hideSoftInputFromWindow(PurchaseDetailActivity_back.this
			...
			if ("".equals(et_price.getText().toString())) {
				Toast.makeText(PurchaseDetailActivity_back.this, "请输入价格",
						Toast.LENGTH_SHORT).show();
				return;
			}
			// 隐藏关闭输入法
			...
			if (Double.parseDouble(et_price.getText().toString()) <= 0) {
				Toast.makeText(PurchaseDetailActivity_back.this, "请输入超过0的价格",
						Toast.LENGTH_SHORT).show();
				return;
			}
		...
		case R.id.iv_baojia_chakantupian:
			if (ossImagePaths.size() > 0) {
				GalleryImageActivity.startGalleryImageActivity(
						PurchaseDetailActivity_back.this, 0, ossImagePaths);

			}
			break;
		case R.id.id_tv_edit_all:
			if (!"".equals(num) && !"".equals(purchaseFormId)) {
				Intent intent = new Intent(PurchaseDetailActivity_back.this,
	...
	public void quoteRevoke() {
		...
		if ("".equals(sellerQuoteJson_id)) {
			Toast.makeText(PurchaseDetailActivity_back.this, "error",
					Toast.LENGTH_SHORT).show();
			return;
		}
		FinalHttp finalHttp = new FinalHttp();
	...
	public void quoteSave() {
		...
		params.put("cityCode", GetCodeByName.initProvinceDatasDocument(
				PurchaseDetailActivity_back.this, mCurrentProviceName,
				mCurrentCityName));
		...
		finalHttp.post(GetServerUrl.getUrl() + "admin/quote/save", params,
				new AjaxCallBack<Object>() {
					...
					public void onSuccess(Object t) {
						// TODO Auto-generated method stub
						if (hud != null
								&& !PurchaseDetailActivity_back.this.isFinishing()) {
							hud.dismiss();
						}
						try {
							JSONObject jsonObject = new JSONObject(t.toString());
...
//								finish();
	...
	private void showCitys() {
		...
		tv_sure.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				tv_address.setText(mCurrentProviceName + "\u0020"
						...
						+ "\u0020");
				if (!PurchaseDetailActivity_back.this.isFinishing()
						&& dialog != null) {
					if (dialog.isShowing()) {
						dialog.cancel();
					} else {
		...
		if (!PurchaseDetailActivity_back.this.isFinishing() && dialog.isShowing()) {
			dialog.cancel();
		} else if (!PurchaseDetailActivity_back.this.isFinishing() && dialog != null
				&& !dialog.isShowing()) {
			dialog.show();
		}
	...
	private void setUpData() {
		...
		mViewProvince.setViewAdapter(new ArrayWheelAdapter<String>(
				PurchaseDetailActivity_back.this, mProvinceDatas));
		// 设置可见条目数量
	...
	public void onGeocodeSearched(GeocodeResult result, int rCode) {
		...
		if (rCode == 1000) {
			if (result != null && result.getGeocodeAddressList() != null
				...
				Toast.makeText(PurchaseDetailActivity_back.this, R.string.no_result,
						1).show();
			}

		} else {
			Toast.makeText(PurchaseDetailActivity_back.this, rCode, 1).show();
		}
	}
